...
I learned that making PPO training run stably is only one part of LLM alignment; the quality of the preference data, the reward model, the stopping behavior, and the evaluation protocol can matter just as much as the optimizer. This project gave me a working system in which those interactions are visible and measurable, and a concrete foundation for better-controlled alignment experiments. After training, I used the fine-tuned aligned model to generate responses to validation prompts and evaluate its performance using the trained reward model. For human review, I built a response explorer app to compare the outputs of the base and aligned models side by side.

**[Find the full interactive Base vs PPO response explorer here]({{ '/projects/rlhf-comparison/' | relative_url }})**.
